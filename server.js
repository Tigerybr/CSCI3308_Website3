...
app.get('/team_stats', function(req, res)){
    ...
    db.task('get-everything', task => {
    	return task.batch([task.any(query1), task.any(query2), task.any(query3)]);
    })
...
app.get('/player_info/post', function(req, res){
  var i = req.query.player_choice;
  var query1 = 'SELECT id,name FROM football_players';
  var query2 = 'SELECT * FROM football_players WHERE id=' + i;
  var query3 = 'SELECT COUNT (*) FROM (SELECT id FROM football_players,football_games WHERE football_players.id = ANY(football_games.players)) AS subq WHERE subq.id=' + i;
  db.task('get-everything', task => {
    return task.batch([task.any(query1), task.any(query2), task.any(query3)]);
  })
  .then(data => {
